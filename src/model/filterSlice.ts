...
const createAllergenReducers = (filterKey: 'pageAllergenFilter' | 'drawerAllergenFilter') => ({
    toggleActive: (state: FilterState, action: PayloadAction<boolean>) => {
    ...
    addAllergen: (state: FilterState, action: PayloadAction<string>) => {
...
const createToggleReducer =
    <T extends ToggleableItem>(key: keyof FilterState) =>
    (state: FilterState, action: PayloadAction<T>) => {
